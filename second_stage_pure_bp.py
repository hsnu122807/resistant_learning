...
# light = ""
# sigma = "2"
# dir_target = dir_path + r"\TensorFlow_input_detection_rule_"+rule+"_"+data_amount+"_and_ntu_"+ntu+"_benign_"+data_amount+light+"_no_label"+"_sigma_" + sigma

owl_rule = "2"
data_amount = "100"
sigma = "2"
dir_target = dir_path + r"\19_owl_rules\owl_rule_"+owl_rule+"_"+data_amount+"_and_benign_"+data_amount+"_sigma_" + sigma
...
major_data_x = training_data_result[:int(int(data_amount)*1.9), 3:]
major_data_y = training_data_result[:int(int(data_amount)*1.9), 2].reshape((-1, 1))
...
while hidden_node_is_not_enough:
    ...
    while not_in_tolerance:
        ...
        if (counter % 10000) == 0:
            print(max(current_stage_squared_residuals))
        ...
        if all(squared_residual ** 2 < squared_residual_tolerance ** 2 for squared_residual in current_stage_squared_residuals):
            print(max(current_stage_squared_residuals))
            ...
            # print(major_data_x)
            # print(major_data_y)
            for i in range(major_data_x.shape[0]):
                if major_data_y[i] == -1:
                    major_data_x_mal_part = np.append(major_data_x_mal_part, major_data_x[i])
                if major_data_y[i] == 1:
                    major_data_x_benign_part = np.append(major_data_x_benign_part, major_data_x[i])
